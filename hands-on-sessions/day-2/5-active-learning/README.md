...
* Dive into the configuration space generated by active learning** (Optional)
...
and atomic forces computed with respect to all the four models (`devi_v` and `devi_f`). By default only the maximal, minimal and average model deviations are output to the ```md.out``` file.
...
``` get_configurations.py ``` script available at ``` $TUTORIAL_PATH/hands-on-sessions/day-2/5-active-learning/template/iteration/_explore/_scripts ```. In each of the directories, run the script as ``` python get_configurations.py ```. This will create a directory called ``` extracted-confs ``` which will have the following Quantum Espresso input files ``` pw-si-?.in ``` where the ``` ? ``` corresponds to the indices of the different configurations.

### Labeling step
In the ``` extracted-confs ``` directory perform DFT calculations of the Labeled configurations using the Quantum Espresso software package [(see hands-on session 2)](https://github.com/CSIprinceton/workshop-july-2023/tree/main/hands-on-sessions/day-1/2-quantum-espresso). To run these calculations use the slurm shell script ``` job.sh ``` available at  ``` $TUTORIAL_PATH/hands-on-sessions/day-2/5-active-learning/template/iteration/_explore/_scripts ```. Once the calculations are complete, you should see the Quantum Espresso output files ``` pw-si-?.out ``` generated.
...
## Dive into the configuration space generated by active learning** (Optional)
In this section, we will perform a data analysis on the explored structures in the active learning protocol.

1. Get the latest repository.
``` bash
...
2. Create a new conda environment and restart jupyter notebook. (Take ~3 minutes.)
``` bash
# create and activate new env
$ conda create -n dscribe python=3.10 dscribe notebook nb_conda_kernels -c conda-forge
$ conda activate dscribe
# restart jupyter notebook
$ nohup jupyter notebook --port=2333 &
# If you already had one, kill the current one first
$ ps
    PID TTY          TIME CMD
  31437 pts/1    00:00:00 bash
  31985 pts/1    00:00:00 jupyter-noteboo
  32079 pts/1    00:00:00 ps
$ kill 31985

```

3. Check the jupyter notebooks in `./insights`

4. How to compute `max_devi_f` using python scripts (for notebook `plot_configuration_space_with_deviation.ipynb`)
``` bash

# go to the exploration directory
$ cd ./extracted-confs

# This generates a file `./configurations.xyz` that stores explored structures, 
# which you can download to visualise in the OVITO.
$ python ./convert_data.py

# This generates a file `./max_devi_f.txt` that stores the force deviations.
# NOTE: This requires the `dp` env
$ conda activate dp 
# NOTE: When you first run this script, it takes several seconds to load dependancies.
$ python ./compute_devi.py --structure ./configurations.xyz --model ./frozen_1.pb ./frozen_2.pb ./frozen_3.pb ./frozen_4.pb
